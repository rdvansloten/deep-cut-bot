...
class salmon_run(app_commands.Group):
  ...
  async def subscribe(self, interaction: discord.Interaction):
    await interaction.response.send_message(subscribe.subscribe_channel(guild_id=interaction.guild.id, 
...
async def salmon_run_schedule():
  ...
  if True:
    if salmon_run_schedule['setting']['coopStage']['name']:
      ...
      for i in salmon_run_schedule["setting"]["weapons"]:        
        embed = discord.Embed(description=get_html.get_weapon_description(f"https://splatoonwiki.org/wiki/{i['name'].replace(' ', '_')}"))
        # Set the thumbnail
